function mainCtrlFn( $scope, mainService, $rootScope, $interval, $timeout, $location, shareDataFtry ) {
	...
	(function () { // main Init
		getUsers( function () {
			...
			$scope.current.supplier = $scope.users[supplierIndex];
			$scope.users.splice( supplierIndex, 1 );
			if ( $rootScope.credentials.admin ) { // supplier logged
				$scope.selectedClientModel = $scope.users[0];
...
// 		}, 3000, 555);
// $scope.fn1 = function() { $location.path('/template')};



}

mainCtrlFn.$inject = [ '$scope', 'mainService', '$rootScope', '$interval', '$timeout', '$location', 'shareDataFtry' ];
